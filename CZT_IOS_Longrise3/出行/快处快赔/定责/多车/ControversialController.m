...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(getDataFromNextPage:) name:@"GetDataFromNextPage" object:nil];
...
#pragma mark 返回键点击回调函数
-(void)onClick:(UIButton *)btn
{
    static int i = 0;
    NSMutableArray *navigationArray = [[NSMutableArray alloc] initWithArray: self.navigationController.viewControllers];
    for (int j = 0; j < navigationArray.count; j++) {
        if ([navigationArray[j]isKindOfClass:[ResponsViewController class]]) {
            i = j;
        }
    }
    [self.navigationController popToViewController:[navigationArray objectAtIndex:i] animated:YES];
}

-(void)getDataFromNextPage:(NSNotification *)notify{
    NSDictionary *userInfo = notify.userInfo;
    self.dutyData = [userInfo objectForKey:@"duty"];
}
