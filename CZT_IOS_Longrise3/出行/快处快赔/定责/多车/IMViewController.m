...
NSString const * monitorIP = @"203.86.8.92:82";  //监听IP
#define SPACE_CELL_HEIGHT 110

@interface IMViewController ()<Mp3RecorderDelegate,AVAudioPlayerDelegate,UINavigationControllerDelegate,TakePhotoViewControllerDelegate,UIScrollViewDelegate>{
    ...
    NSTimer *timer;  //计数器
    int timeCount;   //语音时间
    NSArray *dutyData; //责任类型数组
    NSString * state; //判断是否进入到照相机界面
}

@property (weak, nonatomic) IBOutlet UIButton *sendMessageButton; //发送消息按钮
@property (weak, nonatomic) IBOutlet UIButton *toolButton;   //工具栏按钮
@property (weak, nonatomic) IBOutlet UILabel *warnLabel;     //提醒label
@property (weak, nonatomic) IBOutlet UILabel *overTimeLabel; //超过最大录音时间label
@property (weak, nonatomic) IBOutlet UILabel *recordTimeLabel; //录音时间太短提示label
                    ...
                    if (nil != callBackDic) {
                        if ([callBackDic[@"restate"]isEqualToString:@"0"]) {
                            ...
                            if ([msgType isEqualToString:@"0"]) {
                                ...
                                if (![dic1[@"msgdata"]isEqual:@""]) {
                                    NSDictionary *dataDic = dic1[@"msgdata"];
                                    if (![dataDic[@"dutylist"]isEqual:@""]) {
                                        NSArray *arr = dataDic[@"dutylist"];
                                        [vc setGuideResult:arr];
                                    }
                                }
...
-(void)lookForHistory{
    ...
    [bean setValue:[Globle getInstance].loadDataName forKey:@"username"];
    [bean setValue:[Globle getInstance].loadDataPass forKey:@"password"];
    [[Globle getInstance].service requestWithServiceIP:[Globle getInstance].serviceURL ServiceName:[NSString stringWithFormat:@"%@/zdsearchunreadmsgs",kckpzcslrest] params:bean httpMethod:@"POST" resultIsDictionary:YES completeBlock:^(id result) {
        ...
        if (nil != bigDic) {
            if ([bigDic[@"restate"]isEqualToString:@"0"]) {
                ...
                if (![bigDic[@"data"]isEqual:@""]) {
                    ...
                    //调用接口将未读消息置为已读
                    [[Globle getInstance].service requestWithServiceIP:[Globle getInstance].serviceURL ServiceName:[NSString stringWithFormat:@"%@/zdsetmsgisread",kckpzcslrest] params:bean httpMethod:@"POST" resultIsDictionary:YES completeBlock:^(id result) {
                ...
                if (![bigDic[@"data"]isEqual:@""]) {
                    ...
                    if (![dic[@"msgdata"] isEqual:@""]) {
                        ...
                        [[Globle getInstance].service requestWithServiceIP:[Globle getInstance].serviceURL ServiceName:[NSString stringWithFormat:@"%@/zdreceivermsgcallback",kckpzcslrest] params:bean httpMethod:@"POST" resultIsDictionary:YES completeBlock:^(id result) {
                            ...
                            if (nil != callBackDic) {
                                if ([callBackDic[@"restate"]isEqualToString:@"0"]) {
                                    [self setGuideResult:msgData[@"dutylist"]];
...
//展示交警责任认定消息
-(void)setGuideResult:(NSArray *)dutyList{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (dutyList != nil) {
            dutyData = dutyList;
            self.navigationController.navigationBar.alpha = 0.5;
            self.guideBacView.hidden = NO;
...
-(void)keyBoardWillShow:(NSNotification *)note{
    ...
    keyBoard = - rect.size.height;   //获取键盘高度
...
- (IBAction)camera:(id)sender {
    TakePhotoViewController *TVC = [[TakePhotoViewController alloc]initWithNibName:@"BJJT_Lib.framework/TakePhotoViewController" bundle:nil];
    TVC.currntMark = @"1";
    TVC.delegate = self;
    //发出通知关闭连接
    [[NSNotificationCenter defaultCenter]postNotificationName:NotificationNameForLoadMenu object:@"2"];
...
//接收到交警修改指导意见点击事件
- (IBAction)recieveResultsClicked:(id)sender {
    self.navigationController.navigationBar.alpha = 1;
    self.guideBacView.hidden = YES;
    self.guideView.hidden = YES;
    
    //创建通知
    NSDictionary *dict = [[NSDictionary alloc]initWithObjectsAndKeys:dutyData,@"duty", nil];
    NSNotification *notify = [NSNotification notificationWithName:@"GetDataFromNextPage" object:nil userInfo:dict];
    //发送通知将数据传回
...
-(void)takeTheCamera:(UIImage *)image{
    ...
    imagesize.height = image.size.width/(image.size.height/(240*([UIScreen mainScreen].bounds.size.width/375.0)));
    ...
    [self addPhotoWithContent:image time:sendTime longtime:time];
    [_dispatchTableView reloadData];
    //滚动到当前行
    if (dataArray.count > 1) {
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:dataArray.count-1 inSection:0];
